...
class Controller_Listing extends Controller
{
    public function get_index()
    {
        ...
        $primaryFilterTaxonomy = TaxonomyTerm::get($primaryFilter, "Category");
        if ($primaryFilterTaxonomy == null)
        {
            //  it's not a category, search for other taxonomies
            $primaryFilterTaxonomy = TaxonomyTerm::get($primaryFilter);
        }
        
        if ($primaryFilterTaxonomy == null)
            throw new HTTP_Exception_404();
        
        $taxonomyType = Taxonomy::getFromCacheByGuid($primaryFilterTaxonomy->taxonomyGuid);
        ...
        TPL::setTaxonomyTerm($primaryFilterTaxonomy);
